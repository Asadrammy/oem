...
import { getVehicleType, updateVehicleType } from "@/lib/api";
import {
  Card,
  CardHeader,
  CardContent,
...
export default function EditVehicleTypePage() {
  ...
  useEffect(() => {
    const fetchVehicleType = async () => {
      try {
        const data = await getVehicleType(vehicleTypeId);
        setCode(data.code);
        setName(data.name);
        setCategory(data.category);
        setDrivetrain(data.drivetrain);
        setBatteryCapacity(data.battery_capacity_kwh?.toString() || "");
        setMotorPower(data.motor_power_kw?.toString() || "");
        setWltpRange(data.wltp_range_km?.toString() || "");
        setStatus(data.status);
        setDescription(data.description);
      } catch (e: any) {
        setErr(e.message || "Failed to load vehicle type");
      } finally {
        setLoading(false);
      }
    };
...
const onSubmit = async () => {
  ...
  try {
    ...
    toast({
      title: "Error",
      description: e.message || "Failed to update vehicle type",
      variant: "destructive",
    });
  } finally {
    setSubmitting(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4">
        ...
        <h1 className="text-2xl font-bold text-center">Edit Vehicle Type</h1>
      </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button onClick={onSubmit} disabled={submitting}>
              ...
              {submitting ? "Saving…" : "Update Vehicle Type"}
            </Button>
          </div>
        </CardContent>
      </Card>
